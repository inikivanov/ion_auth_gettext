 ...
 * Modification for work with gettext(): Ivan Ivanov
 *
 * Created:  10.01.2009
 *
        ...
        if ($this->ion_auth_model->forgotten_password($identity)) {
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    $this->set_message(__('Password Reset Email Sent'));
                    return $data;
                } else {
                    ...
                    $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . __('Forgotten Password Verification'));
                    ...
                    if ($this->email->send()) {
                        ...
                        $this->set_error(__('Unable to email the Reset Password link'));
                        return FALSE;
                    }
                }
            } else {
    ...
    public function forgotten_password_complete($code) {
        ...
        if ($new_password) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . __('New Password'));
                $this->email->message($message);
    ...
    public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) { //need to test email activation
        ...
        if (!$email_activation) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
                $this->email->to($email);
                $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . __('Account Activation'));
                $this->email->message($message);

                if ($this->email->send() == TRUE) {
                    $this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful', 'activation_email_successful'));
                    $this->set_message(__('Activation Email Sent. Please check your inbox or spam'));
                    return $id;
                }
            }

            $this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_unsuccessful', 'activation_email_unsuccessful'));
            $this->set_error(__('Unable to Send Activation Email'));
            return FALSE;
        }
    }
