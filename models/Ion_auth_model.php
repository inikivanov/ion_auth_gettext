...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code !== FALSE)
		{
			...
			if ($query->num_rows() !== 1)
			{
				...
				$this->set_error(__('Unable to Activate Account'));
				return FALSE;
			}
		...
		if ($return)
		{
			...
			$this->set_message(__('Account Activated'));
		}
		else
		{
			$this->trigger_events(array('post_activate', 'post_activate_unsuccessful'));
	...
	public function deactivate($id = NULL)
	{
		...
		if (!isset($id))
		{
			$this->set_error(__('Unable to De-Activate Account'));
			return FALSE;
		}
		else if ($this->ion_auth->logged_in() && $this->user()->row()->id == $id)
		{
			$this->set_error(__('You cannot De-Activate your self.'));
			return FALSE;
		}
		...
		if ($return)
		{
			$this->set_message(__('Account De-Activated'));
		}
		else
		{
			$this->set_error(__('Unable to De-Activate Account'));
	...
	public function reset_password($identity, $new) {
		...
		{
			$this->trigger_events(array('post_change_password', 'post_change_password_unsuccessful'));
			$this->set_error(__('Unable to Change Password'));
			return FALSE;
		}
		...
		if ($return)
		{
			...
			$this->set_message(__('Password Successfully Changed'));
		}
		else
		{
			$this->trigger_events(array('post_change_password', 'post_change_password_unsuccessful'));
	...
	public function forgotten_password_complete($code, $salt = FALSE)
	{
		...
		if ($profile)
		{
			...
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0)
			{
				...
				{
					//it has expired
					$this->set_error(__('Forgot pass expire'));
					$this->trigger_events(array('post_forgotten_password_complete', 'post_forgotten_password_complete_unsuccessful'));
					return FALSE;
				}
			}
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_check($identity))
		{
			$this->set_error(__('Identity Already Used or Invalid'));
			return FALSE;
		}
		else if (!$this->config->item('default_group', 'ion_auth') && empty($groups))
		{
			$this->set_error(__('Default group is not set'));
			return FALSE;
		}
		...
		{
			$this->set_error(__('Invalid default group name set'));
			return FALSE;
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		{
			$this->set_error(__('Incorrect Login'));
			return FALSE;
		}
		...
		if ($this->is_max_login_attempts_exceeded($identity))
		{
			...
			$this->set_error(__('Temporarily Locked Out.  Try again later.'));
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				{
					$this->trigger_events('post_login_unsuccessful');
					$this->set_error(__('Account is inactive'));
				...
				$this->set_message(__('Logged In Successfully'));
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column])
		{
			...
			$this->set_error(__('Unable to Update Account Information'));
		...
		$this->set_message(__('Account Information Successfully Updated'));
		return TRUE;
	}
	...
	public function delete_user($id)
	{
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			$this->set_error(__('Unable to Delete User'));
			return FALSE;
		}
		...
		$this->set_message(__('User Deleted'));
		return TRUE;
	}
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		if(!$group_name)
		{
			$this->set_error(__('Group name is a required field'));
			return FALSE;
		}
		...
		if($existing_group !== 0)
		{
			$this->set_error(__('Group already exist'));
			return FALSE;
		}
		...
		// report success
		$this->set_message(__('Group created Successfully'));
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			{
				$this->set_error(__('Group name already taken'));
				return FALSE;
			}
		...
		{
			$this->set_error(__('Admin group name can not be changed'));
			return FALSE;
		}
		...
		$this->set_message(__('Group details updated'));
	...
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->name == $this->config->item('admin_group', 'ion_auth'))
		{
			...
			$this->set_error(__('Can\'t delete the administrators\' group'));
			return FALSE;
		}
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			$this->set_error(__('Unable to delete group'));
			return FALSE;
		}
		...
		$this->set_message(__('Group deleted'));
		return TRUE;
	}
